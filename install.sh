...
REP_LINK="https://download.docker.com/linux/ubuntu"
REP_PATH="/etc/apt/sources.list.d/$REP_NAME.list"
REP_INFO="deb [arch=$ARCH signed-by=$GPG_PATH] $REP_LINK $VERSION_CODENAME stable"
...
run_cmd "Installing $PKG_NAME... " \
    ...
    bash -c "apt-get install -qq $PKG_NAME"

run_cmd "Updating $SUDO_USER group... " \
    "Failed to update $SUDO_USER group." \
